...
export interface OrgEvent {
  ...
  type?: string;
  color?: string;
  resource?: string; // website, yt video, or any resource
  custom_data?: Record<string, unknown>;
  created_at: string;
  updated_at: string;
}

// Event with org_customer info (returned by API GET endpoints)
...
export interface CreateEventRequest {
  title: string;
  description?: string;
  startTime?: Date | string; // Made optional - events can be instant actions
  endTime?: Date | string;
  repeat?: "daily" | "weekly" | "monthly" | "yearly" | string;
  type?: string;
  color?: string; // hex color for calendar GUI
  resource?: string; // URL or resource identifier
  customData?: Record<string, unknown>; // Custom data for the event
  user: LoomCalUser;
...
// Event deduplication and saving control
...
// Complex uniqueness criteria with logical operators
export interface UniquenessCriteria {
  ...
  resource?: boolean;
  customData?: boolean;
  // User-related fields for uniqueness
  user?: boolean;
  "user.email"?: boolean;
  "user.name"?: boolean;
...
// Individual field uniqueness specification
